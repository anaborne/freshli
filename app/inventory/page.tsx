...
/** A recognised row is uploadable only once every field the table needs is filled. */
...
const isComplete = (item: DraftIngredient) =>
  Boolean(item.name && item.quantity && item.unit && item.expirationDate && item.category);
...
export default function InventoryPage() {
    ...
    const handleManualSubmit = async (e: React.FormEvent) => {
        ...
        // A lookup failure must not be read as "no such row" and silently turn an
        // update into an insert, which is how the same stock ends up duplicated.
        if (lookupError) {
            console.error('Error looking up existing ingredient:', lookupError.message);
            setIsSaving(false);
            return;
        }
    ...
    const updateIngredient = (index: number, newItem: DraftIngredient) => {
      setRecognized(prev =>
